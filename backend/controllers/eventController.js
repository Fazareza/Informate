...
exports.updateEvent = async (req, res) => {
  try {
    ...
    const [oldData] = await db.query(
      "SELECT banner_image FROM events WHERE event_id = ?",
      [id]
    );
    if (oldData.length === 0)
      return res.status(404).json({ message: "Event tidak ditemukan" });
    ...
    res
      .status(200)
      .json({ success: true, message: "Event berhasil diperbarui" });
  } catch (error) {
    res
      .status(500)
...
exports.deleteEvent = async (req, res) => {
  try {
    ...
    const [check] = await db.query("SELECT * FROM events WHERE event_id = ?", [
      id,
    ]);
    if (check.length === 0) {
      return res.status(404).json({ message: "Event tidak ditemukan" });
